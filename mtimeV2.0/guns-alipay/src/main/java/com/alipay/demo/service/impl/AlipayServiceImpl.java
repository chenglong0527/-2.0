...
//        String totalAmount = "0.01";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("D:\\Develop\\Coding WorkingSpace\\pic/qr-%s.png",
...
//                qrCodeImge.delete();
    ...
    @Override
    public BaseRespVo getPayResult(String orderId) {
        String msg = query(orderId);
        if (msg.equalsIgnoreCase("success")){
            Integer orderStatu = orderService.getPayResult(orderId);
            if (orderStatu==1){
                HashMap<String, Object> map = new HashMap<>();
                map.put("orderId",orderId);
                map.put("orderStatus",orderStatu);
                map.put("orderMsg","支付成功");
                return BaseRespVo.ok(map);
            }else {
                return BaseRespVo.fail(1,"支付失败");
            }
        }
        return BaseRespVo.fail(1,msg);
    }
